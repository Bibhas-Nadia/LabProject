...
var CONFIG = {
  ...
  // Optional e-mail alerts. Leave "" to switch OFF.
  NOTIFY_EMAIL: "",            // e.g. "ashoklab86@gmail.com"  -> you get an e-mail for every booking/feedback
  SEND_PATIENT_EMAIL: false,   // true -> patient also gets a confirmation e-mail (Gmail limit ~100 mails/day)
...
function handleFeedback_(d) {
  ...
  try {
    ...
    sendFeedbackMail_({ id: id, name: name, email: email, rating: rating, message: message });
...
function sendBookingMails_(b) {
  try {
    ...
    if (CONFIG.SEND_PATIENT_EMAIL && b.email) {
      MailApp.sendEmail({
        to: b.email, name: CONFIG.LAB_NAME,
        subject: mailSafe_("Your booking " + b.ref + " - " + CONFIG.LAB_NAME),
        body: ["Dear " + b.name + ",", "",
          "Your request is received.", "",
          "Ref: " + b.ref, "Service: " + b.service, (b.item !== b.service ? "Item: " + b.item : ""),
          "Date: " + b.dateReadable, "Time: " + b.slot, "",
          "We will call you on " + b.phone + " to confirm.",
          "Need help? Call " + CONFIG.LAB_PHONE + ".", "", CONFIG.LAB_NAME].filter(function (x) { return x !== ""; }).join("\n")
      });
    }
  } catch (err) {
    logEvent_("mail", "MAIL_ERROR", String(err && err.message ? err.message : err), "");   // mail problems never block a booking
  }
}
...
function sendFeedbackMail_(f) {
  try {
    ...
    logEvent_("mail", "MAIL_ERROR", String(err && err.message ? err.message : err), "");
  }
}
